...
class SingleBlockProcessor(DataProcessor):
    ...
    def forward(self, data):
        ...
        slices = tuple([slice(0, min(b, s)) for b, s in zip(retval.shape, data.shape)])
    ...
    def init_data(
        ...
        if split is not None:
            assert (
                not train_idx and not test_idx
            ), "Do not use both split and manual index"
...
class VolumeBlockProcessor(DataProcessor):
    ...
    def forward(self, data):
        ...
        lac_blocks = volumeblocks.split(
            data, block_shape_big, binning=self.binning, sampling=self.sampling
        )
        return lac_blocks
    ...
    def init_data(
        ...
        for ind in train_idx:
            ...
            blocks_y = volumeblocks.split_label(
                labels, block_shape_big, binning=self.binning, sampling=self.sampling
            )
...
class RandomBlockProcessorRefineMask(DataProcessor):
    ...
    def init_data(
        ...
        for ind in test_idx:
            ...
            blocks_x = volumeblocks.split(
                data_in, block_shape_big, binning=self.binning, sampling=self.sampling
            )
...
class FeatureBlockProcessor(DataProcessor):
    def __init__(self, network, block_shape, binning, sampling=1.0):
        super().__init__()
        self.network = network
        self.block_shape = block_shape
        self.binning = binning
    ...
    def forward(self, data):
        ...
        block_shape_big = [s * self.binning for s in self.block_shape]
        lac_blocks = volumeblocks.split(
            input_data, block_shape_big, binning=self.binning, sampling=self.sampling
        )
        return lac_blocks
